...
var network = cc.Class.extend({
	...
	contect:function(func){
	    ...
	    this.websocket.onmessage = function(ev) {
	        ...
	        if (callFunc !== undefined && callFunc !== null){
	        	if (data.p[0] !== undefined && data.p[0] !== null){
	        		console.log("data.p[0]"+data.p[0]);
	        		callFunc(data.p[0]); 
	        	}
	        	else{
	...
	//s 由接口号和参数字段组成的列表，通过socket发送。字符串通过"^&^"来拼接。
	socketCall:function(s){
	...
	onSendMessageSucceed:function(data){
	    ...
	    this.pushEvent("ON_SEND_MESSAGE_SUCCEED",data);
	},
